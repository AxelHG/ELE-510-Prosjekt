\documentclass[10pt]{article}

\usepackage{config/style}

\newcommand{\mytitle}{Object Detection with YOLOv3}
\newcommand{\myauthor}{
\begin{center}
    Axel H. Grøvdal  \\
\end{center}
}

\title{\mytitle}
\author{\myauthor}
\date{\today}

%----- DOCUMENT -----%
\begin{document}

% Title page
\input{title.tex}

% Preface and abstract - edit corresponding file under content/ to change

\pagenumbering{Roman}
\input{content/0-summary.tex}

% Generates table of contents, list of figures, list of tables
\cleardoublepage % Clear floats
\tableofcontents
\cleardoublepage
\listoffigures
\vspace{1cm}

% Main matter - edit corresponding file under content/ to change
\input{content/01-introduction.tex}
\input{content/02-Theory.tex}
\input{content/03-implementation.tex}
\input{content/04-experiments_and_result.tex}

% \input{content/0X-conclusion.tex}  %eksempel på div latex triks.

% Bibliography - edit references.bib and use the \cite command in text
\clearpage
\renewcommand{\bibname}{References}
\bibliographystyle{plain}
\bibliography{references}

% Appendices
\appendix
\addappheadtotoc
\input{appendices/appendices.tex}

% Example text to get started
%\input{example.tex}

\end{document}